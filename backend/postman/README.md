...
- The collections include MCP discovery endpoints: `mcp-manifest`, `mcp-manifest/effective`, `mcp-planning-guide`, `mcp-bootstrap` (including `hardware_smoke_plan` contract checks), `mcp-execution-plan` (including `mode=sync`, `include_disabled_commands=false`, and contract checks for `applied_filters`, `stats` with `stats.filtered_out_steps`, and `hardware_smoke_plan` across default/sync/runnable flows), plus `hardware/readiness` and `hardware/smoke-plan`.
