...
- The collections include MCP discovery endpoints: `mcp-manifest`, `mcp-manifest/effective`, `mcp-planning-guide`, `mcp-bootstrap`, `mcp-execution-plan` (including `mode=sync`, `include_disabled_commands=false`, and contract checks for `applied_filters` + `stats`), and `hardware/readiness`.
