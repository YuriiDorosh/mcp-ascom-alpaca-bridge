...
- TODO(P4-CI-GITHUB): Add GitHub Actions workflow that runs backend Dockerized pytest targets on push/PR so regressions surface before merge. [DONE: `.github/workflows/backend-tests.yml` runs `make test-backend-local` — one container, one Poetry install — over `tests/unit` + `tests/integration` when `backend/` or the workflow file changes]
- TODO(P4-CI-GITHUB-MODEL-SVC): Run model-service pytest in GitHub Actions on push/PR when the microservice tree changes so Kafka/worker regressions surface without local Docker setup. [DONE: `.github/workflows/model-service-tests.yml` runs `make test-local` from `model-service/Makefile` (Python 3.12 + Poetry in one container)]
...
> **Requires physical Seestar:** `P5-HW-SMOKE`, `P5-HW-VALIDATION`, and `make hardware-smoke-live-run` only make sense with the S30 Pro powered, joined to your LAN with a stable reserved IP, and Alpaca reachable on port **32323** (see `backend/README.md` if connectivity flakes). Purely software tasks elsewhere in `TASKS.md` do not need the mount on the bench.
