...
- TODO(P5-UI-MOUNT-JOG-GRADATION): Extend mount jog SPA with **fine-grained ΔRA / ΔDec presets** (~20 tiers from fractions of arcsecond-equivalent drift up to arcminute-class bumps), directional **compass arrows (+Dec axis labels)** contrasting sky-frame motion vs rotated camera previews, English operator copy in the SPA, wider layout, optional **near-no-op** acknowledgement hint when prior≈target. [DONE-hil: `MountNudgePanel.tsx` presets + arrows; `frontend/src/index.css`]
