...
- TODO(P4-CI-GITHUB): Add GitHub Actions workflow that runs backend Dockerized pytest targets on push/PR so regressions surface before merge. [DONE: `.github/workflows/backend-tests.yml` runs parallel `make test-unit-local` and `make test-integration-local` when `backend/` or the workflow file changes]
