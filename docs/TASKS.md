...
- TODO(P4-CI-POETRY-CACHE): Cache Poetry dependencies on GitHub-hosted runners so repeated CI stays fast and cheap. [DONE: `actions/setup-python@v5` with `cache: poetry` and per-service `poetry.lock` paths in `ci.yml`]
