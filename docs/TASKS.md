...
- TODO(P4-CI-GITHUB): Add GitHub Actions workflow that runs backend Dockerized pytest targets on push/PR so regressions surface before merge. [DONE: `.github/workflows/ci.yml` runs backend + model jobs only when `dorny/paths-filter` matches touched paths; `workflow_dispatch` runs both suites; backend step uses `make test-backend-local` unless host-Poetry mode is enabled in the same workflow]
- TODO(P4-CI-GITHUB-MODEL-SVC): Run model-service pytest in GitHub Actions when the microservice tree changes. [DONE: same `ci.yml` `model-pytest` job + `model-service/Makefile` `test-local`]
