...
- TODO(P4-DX-ROOT-README): Add repository root README with quick start, layout pointer, tests, and hardware gate pointer. [DONE: `README.md`, `make help` cross-links]
- TODO(P4-DX-ROOT-README-ENV): Document `cp frontend/.env.example frontend/.env` alongside backend env in root quick start. [DONE: `README.md` + `frontend/README.md` Docker section]
...
- TODO(P5-MOUNT-NUDGE-ICRS): Add **equatorial jog** (`GET /telescopes/mount/icrs-equatorial` + token-gated `POST /telescopes/commands/nudge-equatorial`) backed by Alpaca `RightAscension`/`Declination` read plus normalized slew deltas (RA in sidereal seconds east-positive; Dec in arcseconds north-positive), with Kafka/audit `nudge-equatorial`, MCP manifest + execution-plan entries, and operator SPA compass UI (`MountNudgePanel`) that syncs slew form fields. [DONE: backend `read_mount_icrs_equatorial` / `NudgeMountEquatorialCommand`; frontend mount zone; tests in `test_telescope_command_handlers.py` + `test_telescope_operation_events_api.py`]
- TODO(P5-NUDGE-EQ-DEGREE-CONTRACT): Align **`nudge-equatorial`** limits with astronomical expectations (sidereal-second RA vs sky degrees near δ≈0), widen Pydantic bounds to **ΔRA ±12 sidereal-hour** equivalents, add **RA-only / Dec-only** SPA presets labelled with approximate degrees, call out **`alpaca_live` reachability timeouts** versus “motor silent” workflows, integrate regression tests.[DONE-fix: `NudgeEquatorialRequestSchema`; `MountNudgePanel.tsx`; `README`; integration tests max-RA + over-limit 422]
- TODO(P5-HW-ALPACA-ASYNC-SLEW): Prefer **`SlewToCoordinatesAsync`** + **`Slewing`** polling for Alpaca HTTP mounts (mandatory per ASCOM Alpaca guidance; synchronous slews return **0x400** on hosts like **Seestar S30 Pro**) with **`ALPACA_SLEW_TIMEOUT_SECONDS`** / **`ALPACA_SLEW_POLL_INTERVAL_SECONDS`** env tuning; retain legacy sync slew only when `CanSlewAsync=false`. [DONE-hil: `telescope_client._blocking_slew`; Compose + `.env.example`; README note; mocked adapter coverage]
