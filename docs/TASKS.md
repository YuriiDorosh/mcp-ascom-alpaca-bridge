...
- TODO(P4-CI-POETRY-CACHE): Cache Poetry dependencies on GitHub-hosted runners so repeated CI stays fast and cheap. [DONE: `actions/cache@v4` on `~/.cache/pypoetry` keyed per `backend/poetry.lock` or `model-service/poetry.lock`; `setup-python` does not use `cache: poetry` because that mode requires Poetry on PATH before the action runs (`fix/ci-setup-python-poetry-order`)]
