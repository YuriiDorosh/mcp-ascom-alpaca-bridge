...
- TODO(P4-CI-POETRY-CACHE): Cache Poetry dependencies on GitHub-hosted runners so repeated CI stays fast and cheap. [DONE: `actions/cache` on `~/.cache/pypoetry` keyed per `backend/poetry.lock` or `model-service/poetry.lock`; `setup-python` does not use `cache: poetry` because that mode requires Poetry on PATH before the action runs (`fix/ci-setup-python-poetry-order`)]
- TODO(P4-CI-ACTIONS-VERS): Align CI workflow action majors with current GitHub-maintained releases (Node compatibility, cache, path filters). [DONE: `.github/workflows/ci.yml` pins `actions/checkout@v6`, `actions/setup-python@v6`, `actions/cache@v5`, `dorny/paths-filter@v4`]
